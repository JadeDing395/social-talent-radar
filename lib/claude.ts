...
作品特征：${positionBrief.artwork_features.join("、")}`;
  ...
  if (protocol === "anthropic") {
    ...
    const parsed = toolUse.input as Omit<ScoreOutput, "vision_used" | "region_confidence">;
    ...
    const parsed = JSON.parse(tc.function.arguments) as Omit<ScoreOutput, "vision_used" | "region_confidence">;
    return { ...parsed, vision_used: visionUsed, region_confidence: region };
  }
}
