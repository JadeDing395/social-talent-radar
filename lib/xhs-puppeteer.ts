...
import { createSession, delay, elementScreenshotBase64, DEFAULT_HEADLESS, profilePath, type Session } from "./puppeteer-shared";
...
export async function searchUsers(query: string, opts: SearchOpts = {}): Promise<XHSUser[]> {
  const page = await session.getPage();
  // 关键策略：搜【笔记】而不是搜用户昵称（type=51 召回的全是昵称匹配的废账号），
  // 然后从笔记的作者反向聚合，得到真正在创作相关内容的画师。
  ...
  // 滚 3-5 屏触发 lazy load，让更多笔记加载出来（笔记越多，反推出的作者越多）
  const scrollRounds = Math.max(3, opts.page ?? 1);
  for (let i = 0; i < scrollRounds; i++) {
    await page.evaluate(() => window.scrollBy(0, window.innerHeight * 1.5));
    await delay(1500, 2500);
  }

  // Debug: 看页面长什么样，命中多少 user link
  const debug = await page.evaluate(() => {
    const html = document.body.innerHTML;
    const allUserLinks = document.querySelectorAll("a[href*='/user/profile/']");
    const sampleHrefs = Array.from(allUserLinks).slice(0, 5).map((a) => (a as HTMLAnchorElement).href);
    // 探测页面里有没"登录"按钮/请登录提示(说明实质未登录)
    const bodyText = document.body.innerText.slice(0, 500);
    const hasLoginPrompt = /请登录|登录后查看|登录后可|sign in to/i.test(bodyText);
    return {
      htmlLen: html.length,
      userLinkCount: allUserLinks.length,
      sampleHrefs,
  ...
  console.log(`[XHS] page debug: title="${debug.title}" url=${debug.url} html=${debug.htmlLen}B userLinks=${debug.userLinkCount} loginPrompt=${debug.hasLoginPrompt} samples=${JSON.stringify(debug.sampleHrefs)}`);

  // 3) 如果页面 HTML 长度异常小 或 全无 user link 但有"请登录"提示,主动 throw
  if (debug.userLinkCount === 0) {
    ...
    // userLinks=0 但页面正常 → DOM selector 可能失效,日志已打出 bodyPreview 供排查
    console.log(`[XHS] WARN: userLinkCount=0 but page seems normal. bodyPreview: ${debug.bodyPreview}`);
  }

  const users = await page.evaluate(() => {
    // 累加每个作者贡献的笔记数（出现次数）+ 当前搜索词下笔记标题/描述拼接 → 排序信号
    ...
    links.forEach((link) => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (card.querySelector("img") && (card.textContent ?? "").length > 5) break;
      }

      const avatar = (card.querySelector("img") as HTMLImageElement | null)?.src ?? "";
      const cardText = (card as HTMLElement).innerText ?? "";

      // 昵称：链接自身 text 优先（笔记卡片里 a 是作者名），其次 .nickname
      const linkText = (link as HTMLElement).innerText?.trim() ?? "";
      const nameEl = (card.querySelector(".name, .nickname, [class*='nickname'], [class*='user-name']") as HTMLElement | null);
      const nickname = linkText || nameEl?.innerText?.trim() || cardText.split("\n")[0]?.trim() || "";

      // 笔记标题：常在 .title 或 a[href*='/explore/'] 内
      const noteTitleEl = (card.querySelector("a[href*='/explore/'], a[href*='/discovery/'], .title, [class*='title']") as HTMLElement | null);
      const noteTitle = noteTitleEl?.innerText?.trim() ?? "";

      const entry = acc.get(userId);
      if (entry) {
        entry.noteHits += 1;
        if (noteTitle) entry.noteSnippets.push(noteTitle.slice(0, 60));
      } else {
        acc.set(userId, {
          userId,
          nickname,
          avatarUrl: avatar,
          noteHits: 1,
          noteSnippets: noteTitle ? [noteTitle.slice(0, 60)] : [],
        });
      }
    });

    // 按 noteHits 倒序，hits 多的优先（说明该作者持续发相关内容）
    return Array.from(acc.values()).sort((a, b) => b.noteHits - a.noteHits);
  });

  // 排除当前登录用户自己（如果出现在搜索结果里）
  ...
  console.log(`[XHS] searchNotes parsed ${filtered.length} unique authors for "${query}" (top: ${filtered.slice(0, 3).map((u) => `${u.nickname}×${u.noteHits}`).join(", ")})`);

  // 转成 XHSUser shape（fans/notes 留 0，后面 fetchProfileWithNotes 会补全）
  return filtered.map((u) => ({
    userId: u.userId,
    nickname: u.nickname,
    avatarUrl: u.avatarUrl,
    desc: u.noteSnippets.slice(0, 2).join(" / "),
    fansCount: 0,
    notesCount: 0,
