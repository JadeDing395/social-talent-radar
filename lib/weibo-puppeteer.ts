...
import { createSession, delay, elementScreenshotBase64, profilePath, type Session } from "./puppeteer-shared";
...
export async function searchUsers(query: string, opts: SearchOpts = {}): Promise<WBUser[]> {
  ...
  const url = `${SEARCH_HOST}/weibo?q=${encodeURIComponent(cleanQuery)}&typeall=1&suball=1&Refer=g&page=${opts.page ?? 1}`;
  console.log(`[WB] searchPosts query="${cleanQuery}" (原始: "${query}") → ${url}`);
  await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
  await delay(2000, 3500);

  const debug = await page.evaluate(() => {
    const cards = document.querySelectorAll(".card-wrap").length;
    const userLinks = document.querySelectorAll("a[href*='weibo.com/u/'], a[href*='weibo.com/n/']").length;
    return { cards, userLinks, title: document.title, url: location.href };
  });
  console.log(`[WB] page debug: title="${debug.title}" url=${debug.url} .card-wrap=${debug.cards} userLinks=${debug.userLinks}`);

  const users = await page.evaluate(() => {
    // 从微博正文卡片里反推作者；累加同一作者贡献的微博数 → 排序信号
    ...
    const cards = document.querySelectorAll(".card-wrap, .card, [class*='Feed']");
    cards.forEach((card) => {
      // 作者链接通常是 .name 或 a[nick-name] 或 a[href*='/u/']
      const nameEl = (card.querySelector(
        ".info .name, a[nick-name], a[href*='weibo.com/u/'], a[href*='weibo.com/n/'], a[href*='m.weibo.cn/u/']"
      ) as HTMLAnchorElement | null);
      if (!nameEl) return;
      const href = nameEl.href || "";
      // 匹配多种微博链接格式:
      //   weibo.com/u/12345678     → 数字 uid
      //   weibo.com/12345678       → 数字 uid (8位以上)
      //   weibo.com/n/昵称         → 此时用 nick-name attribute 作为 uid 替代
      //   m.weibo.cn/u/12345678    → 移动端
      const uidMatch =
        href.match(/weibo\.com\/u\/(\d+)/) ||
        href.match(/m\.weibo\.cn\/u\/(\d+)/) ||
        href.match(/weibo\.com\/(\d{8,})(?:\?|$|\/)/);
      const uid = uidMatch?.[1];
      if (!uid) return;

      const avatar = (card.querySelector(".avator img, .avatar img, img") as HTMLImageElement | null)?.src ?? "";
      // 微博正文：.txt 或 [node-type='feed_list_content']
      const contentEl = (card.querySelector(".txt, [node-type='feed_list_content'], [class*='content']") as HTMLElement | null);
      const snippet = (contentEl?.innerText ?? "").trim().slice(0, 80);

      const entry = acc.get(uid);
      if (entry) {
        entry.postHits += 1;
        if (snippet) entry.postSnippets.push(snippet);
      } else {
        acc.set(uid, {
          uid,
          screenName: nameEl.innerText.trim(),
          avatarUrl: avatar.startsWith("//") ? `https:${avatar}` : avatar,
          postHits: 1,
          postSnippets: snippet ? [snippet] : [],
        });
      }
    });

    return Array.from(acc.values()).sort((a, b) => b.postHits - a.postHits);
  });

  console.log(`[WB] searchPosts parsed ${users.length} unique authors for "${query}" (top: ${users.slice(0, 3).map((u) => `${u.screenName}×${u.postHits}`).join(", ")})`);

  return users.map((u) => ({
    uid: u.uid,
    screenName: u.screenName,
    avatarUrl: u.avatarUrl,
    description: u.postSnippets.slice(0, 2).join(" / "),
    followersCount: 0,
    friendsCount: 0,
    statusesCount: 0,
    verified: false,
