...
export default function RadarScanForm({ selectedPlatforms, unloggedInPlatforms }: Props) {
  ...
  const persist = (snapshot: ScanFormSnapshot & { customPosition: string }) => {
    try {
    ...
    <div className="space-y-4">
      ...
      <SectionCard title="基本信息" subtitle="岗位、地域、学历、经验,这些是 AI 评分的基础语境">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className={labelClass}>学历要求（可选）</label>
            <input
              type="text"
              value={education}
              onChange={(e) => setEducation(e.target.value)}
              placeholder="本科及以上 / 不限"
